...
public final class ReaderTOC implements Serializable {
    private static final String TAG = "ReaderTOC";
    private static final long serialVersionUID = 1L;

    private final List<TOCElement> elements;

    private ReaderTOC(final List<TOCElement> rs) {
        this.elements = NullCheck.notNull(rs);
    }

    private static void accumulate(final List<TOCElement> elements, final int indent,
                                   final NavigationTable parent, final NavigationElement e) {
        Log.d(TAG, "accumulate: " + e);
        ...
        if (e instanceof NavigationPoint) {
            ...
            Log.d(TAG, "nav point: " +  content_ref + " → " + title);
            final TOCElement te =
                    new TOCElement(indent, title, content_ref, source_href);
            elements.add(te);

            for (final NavigationElement ec : p.getChildren()) {
                ReaderTOC.accumulate(elements, indent + 1, parent, NullCheck.notNull(ec));
            }
        ...
        if (e instanceof NavigationTable) {
            ...
            Log.d(TAG, "nav table: " + t.getSourceHref() + " " + t.getType() + " → " + t.getTitle());
            ...
            Log.d(TAG, "nav table: " + child_elements.size() + " child elements");
    ...
    public static ReaderTOC fromPackage(final org.readium.sdk.android.Package p) {
        NullCheck.notNull(p);
        Log.d(TAG, "requesting toc");
    ...
    public static final class TOCElement implements Serializable {
        ...
        private TOCElement(final int in_indent, final String in_title,
                           final String in_content_ref, final String in_source_href) {
            indent = in_indent;
            title = NullCheck.notNull(in_title);
            content_ref = NullCheck.notNull(in_content_ref);
            source_href = NullCheck.notNull(in_source_href);
        }
