  ...
  implements ReaderJSONSerializableType
{
  ...
  public double getProgressFractional()
  {
    ...
    final double major_index = (double) page.spine_item_index - 1;
    final double major_max = (double) this.spine_item_count - 1;
    final double major = major_index / major_max;

    final double minor_range = (double) 100 / major_max;

    final double minor_index = (double) page.spine_item_page_index;
    final double minor_max = (double) page.spine_item_page_count;

    // Semi hacky way to make it go all the way to the end
    if ((major_index == (major_max - 1)) && (minor_index == (minor_max - 1))) {
      return 1;
    }

    final double minor;
    if (minor_max > 0) {
      minor = ((minor_index / minor_max) * minor_range) * 0.01;
    } else if (major_index == 0.0) {
      minor = 0;
    } else {
      minor = (minor_range / 2) * 0.01;
    }
